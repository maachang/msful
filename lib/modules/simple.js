...
// len 対象の長さを設定します.
// 戻り値 : 圧縮された情報が返却されます.
o.scomp = function( src,off,len ) {
  var res = o.comp(src,off,len);
  var len = res.length;
  var ret = "";
  for(var i = 0; i < len; i ++) {
...
// len 対象の長さを設定します.
// 戻り値 : 解凍された情報が返却されます.
o.dscomp = function( src,off,len ) {
  var ret = o.dcomp(src,off,len);
  return binaryToUTF8( ret,0,ret.length ) ;
}
