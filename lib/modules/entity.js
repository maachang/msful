...
var _make = function(out, name, value) {
  ...
  if(src == undefined) {
    throw new Error("指定名 \"" + name + "\" のentityは存在しません");
  } else if(typeof(value) != "object") {
    throw new Error("指定名 \"" + name + "\" のデータの型は不正です");
  }
  var e, sub, key, v, def;
  var stack = _stack();
  var len = src.length;
      ...
      switch(e.type) {
        ...
        case -21:
          if((out = stack.pop()) == null) {
            throw new Error("括弧の閉じ位置が不正です:" + name);
          }
          break;
      }
    // 通常条件.
      ...
      v = value[key];
      if((v = checker.convert(e.type, v, true)) == null) {
        v = one.call(key, e.type, v);
      }
      value[key] = v;
    }
  }
