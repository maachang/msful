...
module.exports = function (systemNanoTime) {
  ...
  var _lock = function(name, timeout, call, timeoutCall) {
    ...
    // 読み込み用読み込みロック名.
    var readLockName = lockName + "/" + _READ_LOCK_NAME;
  ...
  var _readLock = function(name, timeout, call, timeoutCall) {
    ...
    // 読み込み用ロックカウント名.
    var readLockCountName = lockName + "/" + _READ_LOCK_NAME + "/" + _LOCK_COUNT_HEAD;
  ...
  var _readUnLock = function(name) {
    ...
    // ロック名 / 読み込み用ロック名が存在する場合.
    if(file.isDir(lockName) || file.isDir(readLockName)) {
  ...
  o.isLock = function(name) {
    ...
    return file.isDir(_baseLockName(name)) ||
      file.isDir(_baseLockName(name) + "/" + _READ_LOCK_NAME)
  }
