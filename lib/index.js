...
exports.createMsFUL = function (port) {
  ...
  var readApi = function (req, res, data, url) {
    ...
    fs.readFile(name, function (err, src) {
      ...
      try {
        if (err) throw err;
        var script = "return (function(_g) {\n" +
          "'use strict';\n" +
          "var require = args.require\n" +
          "var request = args.req\n" +
          "var response = args.res\n" +
          "var headers = args.headers\n" +
          "var params = methods._getPms(request, response, args.data)\n" +
          "var redirect = methods._redirect\n";
        for(var k in _MODULES) {
          script += "var " + k + " = modules." + k + "\n";
        }
        for(var k in _CONFIG) {
          script += "var " + k + " = config." + k + "\n";
        }
        script += "args = null\n" +
          "methods = null\n" +
          "modules = null\n" +
          ...
          "\n})(global)";
        src = null
        var eres = new Function('args', 'methods', 'modules', 'config' , script)(
          { require: require, req: req, res: res, headers: headers, data: data },
          { "_getPms": _getPms, "_redirect": function(url, status) {
  ...
  var readConfig = function() {
    var n,name,p,v
    var list = fs.readdirSync(CONF_DIR);
    var len = list.length;
    for(var i = 0; i < len; i ++) {
      try {
        name = list[i];
  ...
  var defaultModule = function() {
    // JWT.
    _MODULES["jwt"] = Object.freeze(require("./modules/jwt"));
  }
  ...
  // 指定ポートで待つ.
  server.listen(PORT);
  console.info("## listen: " + PORT + " pid:" + process.pid);
}
