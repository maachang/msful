...
exports.createMsFUL = function (port, timeout, contentsCacheMode) {
  ...
  // メモリ別名.
  var _MEMORY = "_$m3M0_3$_$";

  // スクリプト終了フラグ.
  var _EXIT_SCRIPT_FLG = "_$exit_$flg$_";

  // データなし(return),
  var NONE = Object.freeze(new Object());

  // スクリプト終了(throw).
  var EXIT = Object.freeze(new Object());
  ...
  // resultオブジェクト.
  var Result = function(mm) {
    this._m = mm;
  }

  // 正常終了時に呼び出す.
  Result.prototype.success = function(body, status) {
    var m = this._m;
    // スクリプト終了の場合.
    if(m[_EXIT_SCRIPT_FLG]) {
      throw EXIT;
    }
    try {
      if((status|0) <= 0) {
      ...
      if(body != _u && body != null && body != NONE && body != "") {
        body = JSON.stringify(body);
      } else {
        body = "";
      }
      successApi(status, m.headers, m.response, body);
      // スクリプト終了.
      m[_EXIT_SCRIPT_FLG] = true;
    } catch(e) {
      console.debug(e);
    } finally {
      _closeable(m);
  ...
  // エラー出力時に呼び出す.
  Result.prototype.error = function(status, message, e) {
    var m = this._m;
    // スクリプト終了の場合.
    if(m[_EXIT_SCRIPT_FLG]) {
      throw EXIT;
    ...
    try {
      errorApi({status: status, message: message}, status, m.response, e);
      // スクリプト終了.
      m[_EXIT_SCRIPT_FLG] = true;
    } catch(e) {
      console.debug(e);
  ...
  // リダイレクトを行う場合に呼び出す.
  Result.prototype.redirect = function(url, status) {
    try {
      status = status|0
      if(status == 0) {
        status = 303
  ...
  Result.prototype.status = function(status) {
    var m = this._m;
    status = status | 0;
    if (status != 0) {
      m.response.statusCode = status;
  ...
  // スクリプト終了かチェック.
  Result.prototype.isExitScript = function() {
    return this._m[_EXIT_SCRIPT_FLG];
  }
  ...
  // エラーAPI.
  var _errorApi = function(mm) {
    var _m = mm; mm = null;
    return function(e, status) {
      // スクリプト終了の場合.
      if(_m[_EXIT_SCRIPT_FLG]) {
        return;
      }
      try {
        errorApi(e, status, _m["response"], e);
        // スクリプト終了.
        _m[_EXIT_SCRIPT_FLG] = true;
      } catch(ee) {}
    };
  }
  ...
  var _catchsByAsync = function(e, _m) {
    if(e != EXIT) {
      try {
        _m.result.error(500, "internal server error", e);
      } catch(ee) {}
    }
  }
  ...
  // HTTP実行.
  var exec = function (_req, _res, _data) {
    var _rq = _req; _req = null;
    var _rs = _res; _res = null;
    var _dt = _data; _data = null;
    __setImmediate(function(req, res, data) {
      // URLを取得.
      var url = _getUrl(req);
      // アクセス禁止URL.
      ...
      else if (url.indexOf(API_PATH) == 0) {
        // WebAPIのパスチェック.
  ...
  var createMemory = function(req, res, data) {
    ...
    // エラーハンドリング.
    memory._errorApi = _errorApi(memory);

    // httpエラー処理.
    memory.httpError = _httpError;

    // コンソール出力.
    memory.echo = _echo;

    // result処理.
    memory.result = new Result(memory);
  ...
  // crosヘッダ作成.
  var _crosHeaders = function(headers) {
  ...
  var successApi = function(status, headers, res, body) {
    ...
    var crosHeaders = _crosHeaders(headers);
  ...
  // 実行エラー.
  var errorApi = function(e, status, res, err) {
    if (res == _u) {
      return;
    }
    var message = "";
      ...
      fs.stat(apiFile, function(err, stat) {
        try {
          ...
          "(function() {\n" +
            ...
            "return function(_mm){\n" +
              "'use strict';\n" +
              "var m="+_MEMORY+";for(var k in m){if(m[k]!=m){delete m[k];}}\n" +
          ...
          // 操作可能なglobalをセット.
          var _g = {};_g[_MEMORY] = _g;
          // 上書き禁止.
          Object.defineProperty(_g, _MEMORY, {writable : false});
