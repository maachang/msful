...
exports.createMsFUL = function (port) {
  ...
  // リダイレクト.
  var _redirect = function(status, url, res, hed) {
    status = status|0
    if(status == 0) {
      status = 303
    }
  ...
  var setModules = function(out, createOut) {
    ...
    initMod.createModules(out, createOut);
  ...
  var createMemory = function(req, res, data) {
    var memory = {}
    var createModules = {};
    setModules(memory, createModules);
    memory.request = req;
    memory.response = res;
    memory.headers = {};
    ...
    memory.redirect = function(url, status) {
      _redirect(status, url, res, headers);
    };
    ...
    memory.require =  Object.freeze(
      require("./require")(cacheRequire, _MODULES, createModules, _CONFIG));
    return memory;
  }
  ...
  // 正常戻り値処理.
  var successResult = function(status, headers, res, body) {
  ...
  // フィルタ読み込み.
  var readFilter = function(req, res, data, url) {
    var dir = null;
    ...
    var name = constants.API_PATH + dir + constants.FILTER_FILE;
    fs.readFile(name, function (err, src) {
      try {
        ...
        var body = "";
        var bodyLength = 0;
        var closeable = undefined;
        
        // フィルタファイルが存在する場合.
        if (!err) {
          
          // 実行処理.
          closeable = memory.closeable;
          var script = new vm.Script("(function(_g){\n" +
            "'use strict';\n" +
            src + "\n" +
            "})(global);");
          src = null;
          ...
          if (typeof(eres) == "boolean") {
            // 返却が boolean で true の場合のみAPIの実行を行う.
            if(eres) {
              closeable = undefined;
              return readApi(url, memory);
            // falseの場合は、エラー返却.
            } else {
              res.statusCode = 500;
              successResult(status, headers, res, JSON.stringify(
                {error: 500, message: "exit filter."}
              ));
            }
          }
          // 返却情報が存在する場合はAPIの実行は行わない.
          ...
          successResult(status, headers, res, body);
  ...
  // api読み込み.
  var readApi = function (url, memory) {
    if (url.lastIndexOf("/") == url.length - 1) {
      url += "index.js";
    } else if(url.lastIndexOf(".js") != url.length - 3) {
      url += ".js"
    }
    var name = constants.API_PATH + url;
    fs.readFile(name, function (err, src) {
      var status = 200;
      var req = memory.request;
      var res = memory.response;
      var headers = memory.headers;
      var body = "";
      var bodyLength = 0;
      var closeable = undefined;
