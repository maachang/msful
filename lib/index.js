...
exports.createMsFUL = function (port, timeout, contentsCacheMode) {
  ...
  var successApi = function(m, status, body) {
    ...
    // 返却処理.
    return _resultWrite(m, status, body);
  }
  ...
  Result.prototype.success = function(body, status) {
    ...
    try {
      ...
      return successApi(m, status, body);
    } catch(e) {
      console.debug(e);
      return false;
    } finally {
  ...
  Result.prototype.error = function(status, message, e) {
    ...
    try {
      if(message == _u) {
        return errorApi(m, e, status, e);
      } else {
        return errorApi(m, {status: status, message: message}, status, e);
      }
    } catch(ee) {
      console.debug(ee);
      return false;
  ...
  Result.prototype.redirect = function(url, status) {
    try {
      ...
      return m.result.success("", status);
    } catch(e) {
      console.debug(e);
      return false;
    }
          ...
          "(function() {\n" +
            ...
            "return function(_mm){\n" +
              ...
              "} finally {\n" +
                ...
                // unhandledRejection の例外ハンドリングクリア.
