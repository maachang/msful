...
module.exports.createMsFUL = function (port, timeout, contentsCacheMode, args_env, msfulId) {
  ...
  // MSFUL:起動モード用パラメータを取得.
  var _getConfigEnv = function() {
    var ret = _CONFIG[initMod.getEnvironment()];
    if(!ret) {
      ret = _CONFIG;
    }
    return ret;
  ...
  // MSFUL:デバッグモードを取得.
  var _getDebugMode = function() {
    if(_MSFUL_DEBUG === null) {
      var mode = process.env["MSFUL_DEBUG"]
      if(mode) {
        mode = (mode + "").toLowerCase();
        if(mode === "true" || mode === "on") {
          _MSFUL_DEBUG = Object.freeze(true)
        }
      }
    }
    if(_MSFUL_DEBUG === null) {
      _MSFUL_DEBUG = Object.freeze(false);
    }
    return _MSFUL_DEBUG;
  }
  ...
  var _endSendScript = function(m) {
    var ret = true;
    try {
      ret = !("response" in m) || (!(_EXIT_SEND_SCRIPT_FLG in m) && m[_EXIT_SEND_SCRIPT_FLG]) || (!(_EXIT_SEND_ERROR_SCRIPT_FLG in m) && m[_EXIT_SEND_ERROR_SCRIPT_FLG]);
    } catch(e) {
  ...
  var setModules = function(out) {
    ...
    out["config"] = Object.freeze(_CONFIG);
    out["envConf"] = Object.freeze(_getConfigEnv());
    
    out["msfulEnv"] = Object.freeze(initMod.getEnvironment());
    out["msfulId"] = Object.freeze(initMod.getMsfulId());
  }
  ...
  var createMemory = function(req, res, data) {
    ...
    memory.require = Object.freeze(
      require("./require")(cacheRequire, backupRequire, _MODULES, _CONFIG, _getConfigEnv()));
  ...
  var mimeType = function (name) {
    ...
    if (typeof(_getConfigEnv()["mime"]) == "object") {
      var ret = _getConfigEnv()["mime"][n];
      if(ret) {
        return ""+ret;
      }
    }
  ...
  // 実行環境の設定.
  initMod.setEnvironment(args_env);

  // サーバIDの設定.
  initMod.setMsfulId(msfulId);
  ...
  console.info("## listen: " + bindPort + " env:[" + initMod.getEnvironment() + "] timeout:" + (httpTimeout/1000) + "(sec) contentCache:" + _CACHE + " pid:" + process.pid);
  server = null;
}
