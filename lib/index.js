...
exports.createMsFUL = function (port, timeout, contentsCacheMode) {
  ...
  // [スクリプト結果]書き込み処理.
  var _resultWrite = function(m, status, body) {
    try {
      // 書き込み処理.
  ...
  // 次の実行処理をセット.
  Result.prototype.push = function(call) {
  ...
  // スクリプト送信済みかチェック.
  Result.prototype.isSendScript = function() {
    return _endSendScript(this._m);
  }

  // スクリプト送信エラーかチェック.
  Result.prototype.isErrorSendScript = function() {
  ...
  var _errorApi = function(mm) {
    ...
    return function(e, status) {
      try {
        errorApi(_m, e, status, e);
      } catch(ee) {}
    };
  }
  
  // HttpError.
  var _httpError = function(mm) {
    var _m = mm; mm = null;
    return function(status, message, e) {
      try {
        if(message == _u) {
          ...
          errorApi(_m, {status: status, message: message}, status, e);
        }
      } catch(ee) {
        console.debug(ee);
      } finally {
  ...
  var createMemory = function(req, res, data) {
    ...
    // httpエラー処理.
    memory.httpError = _httpError(memory);
        ...
        try {
          ...
          "(function() {\n" +
            ...
            "return function(_mm){\n" +
              ...
              "var _ur = process.once('unhandledRejection', function(reason, p) {\n" +
                "if (reason != EXIT) {\n" +
                  "result.error(500, 'Unknonw Error.', reason);\n" +
                "}\n" +
              "});\n" +
              
              // api実行用メソッドをresult.next実行用にセット.
