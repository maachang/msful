...
module.exports = function (cache, backup, modules, createModules, configs) {
  'use strict';
  ...
  // モジュール再割り当て.
  var reaSetModules = function(out, mod) {
    for(var k in createModules) {
      out[k] = createModules[k];
    }
  }
  ...
  var load = function(name, fs, path, vm, mod) {
    ...
    try {
      ...
      // バックアップから取得.
        ...
        reaSetModules(backupCall.memory, mod);
        var module = {};
      ...
      if (cacheFlag) {
        ...
        // バックアップ.
        backup.cache[name] = {call: call, memory: memory, update: Date.now(), fileTime: mtime};
      }
      return res;
    } catch(e) {
      deleteCache(name);
