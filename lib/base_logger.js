...
module.exports = (function () {
  ...
  var _toDate = function(d) {
    ret = (d.getYear() & 31) | ((d.getMonth() & 31) << 9) | ((d.getDate() & 31) << 18);
  }
    ...
    if(typeNo >= logLevel) {
      setImmediate(function() {
        ...
        fs.stat(outName, function(err, value) {
          ...
          if(!err &&
            ((s > 0 && value.size + ff.length > s) || _toDate(new Date(value.birthtime)) != _toDate(new Date()))) {
            while(true) {
              var mvName = (value.size + ff.length > s) ?
                on + "." + dateString() + "." + cnt + "." + Date.now() + ".log":
                on + "." + dateString() + "." + cnt;
              try {
                // リネーム元が存在しない場合.
                if(!_isFile(on)) {
