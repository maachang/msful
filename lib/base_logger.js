...
module.exports = (function () {
      ...
      setImmediate(function() {
        ...
        if(s > 0) {
          fs.stat(outName, function(err, value) {
            ...
            if(!err &&
              (value.size + f.length > s || _toDate(new Date(value.ctime)) != _toDate(new Date()))) {
              while(true) {
                var mvName = (value.size + f.length > s) ?
                  outName + "." + dateString() + "." + Date.now() + ".log":
                  outName + "." + dateString() + ".log";
                try {
                  // リネーム.
  ...
  o.create = function(name, logLevel, fileSize) {
    ...
    return {
      ...
      warn: function() {
        outLog(_name, _outLogLevel, _maxFileSize, 3, marguments);
      },
