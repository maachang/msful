    ...
    if(typeNo >= logLevel) {
      setImmediate(function() {
        ...
        fs.stat(outName, function(err, value) {
          ...
          var ff = f;
          var mvName = null;
          var bd = value.birthtime;
          var now = new Date();

          // エラーでなく、
          // ファイルサイズの最大値が設定されていて、その最大値が肥える場合.
          // また、作成ファイルの日付の日付が、現在の日付と一致しない場合.
          if(!err &&
            ((s > 0 && value.size + ff.length > s) ||
            ((bd.getYear() & 31) | ((bd.getMonth() & 31) << 9) | ((bd.getDate() & 31) << 18))
              !=
            ...
            while(true) {
              try {
                ...
                mvName = (s > 0 && value.size + ff.length > s) ?
                  on + "." + dateString(bd) + "." + cnt + "." + Date.now() + ".log":
                  on + "." + dateString(bd) + "." + cnt;
