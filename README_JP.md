...
以前筆者は java で動くWebアプリケーションサーバ spring-boot ばかりを利用してきました。またScala言語 Vert.x でWebアプリケーションを構築したりしていました。

最近では、Webアプリケーション環境は node の express を利用して開発もしました。

でも、開発が面倒なんですよね。

何故？って思うかもしれませんが、それは プログラムの修正をしたら、nodeを再起動しなければならないこと。

nodeとかって、スクリプト言語でこの辺のプログラム修正に対して、勝手にnode側が認識して、差し替えてくれる？って思ったのに。

だけど、実際にはnodeを再起動しないと、修正したプログラムは認識されない。

これが「開発時」において、開発効率を低下させているわけですよ。

そして、それらはjavaの spring-boot や scalaの Vert.x これを使ってきた時も同じことを感じたわけです。

PHPの言語やWebサーバとして、別段個人的には良いとは言いませんが、更新されたら、自動で再読込してくれる分PHPの方が、サーバの再起動分が無い分「開発効率」が良いと言えますよね。

そう重い、色々と考え、調査した結果　作れば良い　と思い、そして作りました。

これで少なくとも、nodeサーバの再起動が無くなることで、Webアプリの開発コストはexpressより下がるし、開発スピードも上がると思います。
